...
class Uuid
{
	...
	{
		// Define o padrão da hash v5 (UUID v5)
		$pattern = '/-[0-9a-f]{8}-[0-9a-f]{4}-5[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}/';

		// Encontra todas as ocorrências da hash no final da string
		preg_match_all($pattern, $string, $matches);

		// Caso haja uma ou mais ocorrências da hash no final da string, remova-as
		if (!empty($matches[0])) {
		...
		// Remove espaços em branco do início e fim da string após remover a hash
		$string = trim($string);
