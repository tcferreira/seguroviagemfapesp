...
// Detect admin base_url: use ADMIN_BASE_URL set by router.php (unified server)
// or fall back to BASE_URL env, or auto-detect
if (isset($_SERVER['ADMIN_BASE_URL'])) {
    $config['base_url'] = $_SERVER['ADMIN_BASE_URL'];
} elseif (get_environment('BASE_URL') && get_environment('BASE_URL') !== '') {
    $config['base_url'] = rtrim(get_environment('BASE_URL'), '/') . '/admin/';
} else {
    $config['base_url'] = '';
}
$config['index_page'] = '';
...
$config['multi_company'] = (get_environment("MULTI_COMPANY") == 1 ? true : false);
...
$config['sess_cookie_name'] = 'svfapesp_admin_session';
