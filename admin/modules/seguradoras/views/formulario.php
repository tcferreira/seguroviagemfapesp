                        ...
                        <div class='tab-pane fade show active' id='general'>
                            ...
                            <div class='form-row'>
                                <div class='form-group col-md-6'>
                                    <label for='inputNome'><?php echo T_('Nome da Seguradora'); ?></label>
                                    <input type='text' class='form-control' id='inputNome' placeholder='<?php echo T_('Nome'); ?>' value='<?php echo isset($item) ? $item->nome : ''; ?>' required name='nome'/>
                                </div>
                                <div class='form-group col-md-6'>
                                    <label for='inputLink'><?php echo T_('Link (opcional)'); ?></label>
                                    <input type='text' class='form-control' id='inputLink' placeholder='https://...' value='<?php echo isset($item) ? $item->link : ''; ?>' name='link'/>
                                </div>
                            </div>

                            <!-- ========== UPLOAD DRAG & DROP - Logo ========== -->
                            <div class='form-row'>
                                <div class='form-group col-md-6'>
                                    <label><?php echo T_('Logo da Seguradora'); ?></label>
                                    <input type='hidden' name='image' id='hiddenImage' value='<?php echo isset($item) ? $item->image : ''; ?>'/>
                                    <div class="dropzone-area" id="dropzoneLogo" data-field="image" data-hidden="hiddenImage" data-preview="previewLogo">
                                        <div class="dropzone-content" id="dropzoneLogoContent">
                                            ...
                                            <small class="text-muted">PNG, JPG, WebP, SVG — máx. 10MB</small>
                                        </div>
                                        <div class="dropzone-preview" id="dropzoneLogoPreview" style="display:none;">
                                            <img src="<?php echo (isset($item) && $item->image) ? '/userfiles/seguradoras/'.$item->image : ''; ?>" alt="" id="previewLogo"/>
                                            <button type="button" class="btn btn-sm btn-danger dropzone-remove" title="Remover"><i class="fas fa-times"></i></button>
                                            <div class="dropzone-progress" id="progressLogo" style="display:none;">
                                                <div class="dropzone-progress-bar"></div>
                                            </div>
                                        </div>
                                        <input type="file" class="dropzone-input" id="fileLogo" accept="image/*" style="display:none;"/>
                                    </div>
                                    <?php if(isset($item) && $item->image): ?>
                                    <script>document.addEventListener('DOMContentLoaded',function(){document.getElementById('dropzoneLogoContent').style.display='none';document.getElementById('dropzoneLogoPreview').style.display='flex';});</script>
                                    <?php endif; ?>
                                </div>
                            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    var uploadUrl = '<?php echo site_url("seguradoras/upload_image"); ?>';
